...
public class PrgRunController implements Initializable {
    ...
    @FXML
    TableView<HashMap.Entry<Integer, Pair<Integer, List<Integer>>>> semaphoreTable;
    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphoreTableIndex;
    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphoreTableValue;
    @FXML
    TableColumn<HashMap.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphoreTableIntegers;
    @FXML
    ...
    public void setBarrierTable(){
        ObservableList<HashMap.Entry<Integer, Pair<Integer, List<Integer>>>> semaphoreTableList = FXCollections.observableArrayList();
        semaphoreTableIndex.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getKey().toString()));
        semaphoreTableValue.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().getFirst().toString()));
        semaphoreTableIntegers.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().getSecond().toString()));
        semaphoreTableList.addAll(myController.getRepo().getPrgList().get(0).getSemaphoreTable().getDict().entrySet());
