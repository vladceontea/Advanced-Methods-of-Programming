...
public class PrgRunController implements Initializable {
    ...
    @FXML
    TableColumn<HashMap.Entry<Integer,Integer>, String> latchTableVariable;
    @FXML
    ...
    public void setLatchTable(){
        ...
        latchTableVariable.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getKey().toString()));
        latchTableValue.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().toString()));
        latchTableList.addAll(myController.getRepo().getPrgList().get(0).getLatchTable().getTable().entrySet());
